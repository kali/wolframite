...
pub fn process<R: io::Read>(input: R, output: &path::Path) -> WikiResult<()> {
    ...
    for line in input.lines() {
        ...
        if part.is_none() ||
           (counter % 1000 == 0 && try!(fs::metadata(&path)).len() > 250_000_000) {
        ...
        if line == "[" || line == "]" {
            ...
            let value: serde_json::value::Value = serde_json::from_str(&*line)
                .map_err(|e| {
                             println_stderr!("##### JSON ERROR @{} #####", counter);
                             println_stderr!("{:?}", e);
                             let _ = io::copy(&mut io::Cursor::new(line.as_bytes()),
                                              &mut io::stderr());
                             e
                         })?;
            let mut message = Builder::new_default();
            {
                ...
                try!(job.or_else(|e| {
                                     ...
                                     println_stderr!("{:?}", e);
                                     try!(serde_json::ser::to_writer_pretty(&mut io::stderr(),
                                                                            &value));
                                     return Err(e);
                                 }));
            }
...
fn consume_item<T: Allocator>(value: &serde_json::value::Value,
                              message: &mut Builder<T>)
                              -> WikiResult<()> {
    ...
    match typ.as_str() {
        ...
        _ => {
            Err(format!("type expected to be a string (\"item\", or \"property\") got: {:?}",
                        typ))?
        }
    };
    try!(value
             .pointer("/labels")
             .map(|labels| build_map_to_mono(labels, entity.borrow().init_labels()))
             .unwrap_or(Ok(())));
    try!(value
             .pointer("/descriptions")
             .map(|vs| build_map_to_mono(vs, entity.borrow().init_descriptions()))
             .unwrap_or(Ok(())));
    try!(value
             .pointer("/sitelinks")
             .map(|vs| {
                      build_map(vs,
                                entity.borrow().init_sitelinks(),
                                |json, entry| build_sitelink(json, try!(entry.get_value())))
                  })
             .unwrap_or(Ok(())));
    try!(value
             .pointer("/aliases")
...
fn build_map_to_mono(map_of_maps: &serde_json::value::Value,
                     map: Map::Builder<text::Owned, MongolingualText::Owned>)
                     -> WikiResult<()> {
    build_map(map_of_maps,
              map,
              |json, entry| build_monolingual_text(json, try!(entry.get_value())))
}

fn build_map<F, V>(map_of_maps: &serde_json::value::Value,
                   mut map: Map::Builder<text::Owned, V>,
                   f: F)
                   -> WikiResult<()>
    where V: for<'x> traits::Owned<'x>,
          F: Fn(&serde_json::value::Value, MapEntry::Builder<text::Owned, V>) -> WikiResult<()>
{
    let map_of_maps = try!(map_of_maps
                               .as_object()
                               .ok_or("map of monolingual text is expected as json object"));
...
fn build_monolingual_text(json: &serde_json::value::Value,
                          mut builder: MongolingualText::Builder)
                          -> WikiResult<()> {
    json.pointer("/language")
        .and_then(|v| v.as_str())
        .map(|v| builder.set_language(v));
    if json.pointer("/removed").is_some() {
        builder.set_removed(());
    } else {
        json.pointer("/value")
            .and_then(|v| v.as_str())
            .map(|v| builder.set_value(v));
    }
    Ok(())
}

fn build_sitelink(json: &serde_json::value::Value,
                  mut builder: SiteLink::Builder)
                  -> WikiResult<()> {
    json.pointer("/site")
        .and_then(|v| v.as_str())
        .map(|v| builder.set_site(v));
    json.pointer("/title")
        .and_then(|v| v.as_str())
        .map(|v| builder.set_title(v));
    Ok(())
}
...
fn build_claim(json: &serde_json::value::Value, mut builder: Claim::Builder) -> WikiResult<()> {
    ...
    match json.pointer("/type").and_then(|v| v.as_str()) {
        ...
        Some(e) => return Err(WikiError::from(format!("unexpected value for claim type {}", e))),
        _ => (),
    }
    match json.pointer("/rank").and_then(|v| v.as_str()) {
        ...
        Some(e) => return Err(WikiError::from(format!("unexpected value for claim rank {}", e))),
        _ => (),
    }
    let snak = try!(json.pointer("/mainsnak")
                        .ok_or("I do expect a mainsnak."));
    try!(build_snak(snak, builder.init_mainsnak()));
    Ok(())
}

fn build_snak(json: &serde_json::value::Value, mut builder: Snak::Builder) -> WikiResult<()> {
    json.pointer("/property")
        .and_then(|v| v.as_str())
        .map(|v| builder.set_property(v));
    json.pointer("/datatype")
        .and_then(|v| v.as_str())
        .map(|v| builder.set_datatype(v));
    let snaktype = try!(json.pointer("/snaktype")
                            .and_then(|v| v.as_str())
                            .ok_or("expect a snaktype"));
    match snaktype {
        "value" => {
...
fn build_data_value(json: &serde_json::value::Value,
                    mut builder: DataValue::Builder)
                    -> WikiResult<()> {
    let t = try!(json.pointer("/type")
                     .and_then(|v| v.as_str())
                     .ok_or("expect a type"));
    let v = try!(json.pointer("/value").ok_or("expect a value"));
    match t {
        "string" => builder.set_string(try!(v.as_str().ok_or("expected a string"))),
        "wikibase-entityid" => try!(build_wikibase_entity_ref(v, builder.init_wikibaseentityid())),
        ...
        e => Err(format!("unexpected datavalue type:{} {:?}", e, json))?,
    }
    Ok(())
}

fn build_wikibase_entity_ref(json: &serde_json::value::Value,
                             mut builder: WikibaseEntityRef::Builder)
                             -> WikiResult<()> {
    let typ = try!(json.pointer("/entity-type")
                       .and_then(|v| v.as_str())
                       .ok_or("expect an entity-type"));
    builder.set_type(try!(build_entity_type(typ)));
    builder.set_id(try!(json.pointer("/numeric-id")
                            .and_then(|v| v.as_u64())
                            .ok_or("expect a numeric id")) as u32);
    Ok(())
}
...
fn build_time(json: &serde_json::value::Value, mut builder: Time::Builder) -> WikiResult<()> {
    json.pointer("/time")
        .and_then(|v| v.as_str())
        .map(|v| builder.set_time(v));
    json.pointer("/calendarmodel")
        .and_then(|v| v.as_str())
        .map(|v| builder.set_calendarmodel(v));
    json.pointer("/precision")
        .and_then(|v| v.as_u64())
        .map(|v| builder.set_precision(v as u8));
    json.pointer("/timezone")
        .and_then(|v| v.as_u64())
        .map(|v| builder.set_timezone(v as i16));
    json.pointer("/before")
        .and_then(|v| v.as_u64())
        .map(|v| builder.set_before(v as u64));
    json.pointer("/after")
        .and_then(|v| v.as_u64())
        .map(|v| builder.set_after(v as u64));
    Ok(())
}

fn build_quantity(json: &serde_json::value::Value,
                  mut builder: Quantity::Builder)
                  -> WikiResult<()> {
    json.pointer("/amount")
        .and_then(|v| v.as_f64())
        .map(|v| builder.set_amount(v));
    json.pointer("/lowerBound")
        .and_then(|v| v.as_f64())
        .map(|v| builder.set_lower_bound(v));
    json.pointer("/upperBound")
        .and_then(|v| v.as_f64())
        .map(|v| builder.set_upper_bound(v));
    json.pointer("/unit")
        .and_then(|v| v.as_str())
        .map(|v| builder.set_unit(v));
    Ok(())
}

fn build_globecoordinate(json: &serde_json::value::Value,
                         mut builder: GlobeCoordinate::Builder)
                         -> WikiResult<()> {
    json.pointer("/latitude")
        .and_then(|v| v.as_f64())
        .map(|v| builder.set_latitude(v));
    json.pointer("/longitude")
        .and_then(|v| v.as_f64())
        .map(|v| builder.set_longitude(v));
    json.pointer("/altitude")
        .and_then(|v| v.as_f64())
        .map(|v| builder.set_altitude(v));
    json.pointer("/precision")
        .and_then(|v| v.as_f64())
        .map(|v| builder.set_precision(v));
    json.pointer("/globe")
        .and_then(|v| v.as_str())
        .map(|v| builder.set_globe(v));
    Ok(())
}
